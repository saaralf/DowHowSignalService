   ...
   bool WildMatchCI(const string text_raw, const string pattern_raw)
   {
      ...
      while(t < tl)
      {
         if(p < pl)
         {
            ...
            if(pc=='?' || pc==tc) { t++; p++; continue; }
            if(pc=='*') { star=p; match=t; p++; continue; }
         }

         if(star != -1) { p = star + 1; match++; t = match; continue; }
         return false;
      }

      // Restliche * erlauben
      while(p < pl && (ushort)StringGetCharacter(pat, p) == '*') p++;
      return (p == pl);
   }
...
public:
   ...
   bool Validate()
   {
      if(webhook_system == "")
      {
         Print("WebhookRouter: ERROR: InpWebhook_system ist leer.");
         if(require_known) return false;
      }
      ...
      for(int i=0;i<ArraySize(routes);i++)
      {
         ...
         {
            PrintFormat("WebhookRouter: ERROR: Route unvollständig: key='%s' webhook='%s'",
                        routes[i].key, routes[i].webhook);
            if(require_known) return false;
         }
      }
      return true;
   ...
   string GetWebhookFor(const string broker_symbol)
   {
      ...
      if(idx >= 0) return routes[idx].webhook;
      ...
      for(int i=0;i<ArraySize(routes);i++)
      {
         for(int j=0;j<ArraySize(routes[i].patterns);j++)
         {
            ...
            if(WildMatchCI(s1, pat)) return routes[i].webhook;
            if(n!="" && WildMatchCI(n, pat)) return routes[i].webhook;
         }
      }

      // 5) fallback
      PrintFormat("WebhookRouter: WARN: Kein Mapping für _Symbol='%s' (norm='%s', fx='%s') -> system",
                  sym_raw, n, fx);
